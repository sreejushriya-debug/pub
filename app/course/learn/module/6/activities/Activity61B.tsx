...
import { ArrowRight, Layers, CheckCircle2, XCircle, RotateCcw, Eye } from 'lucide-react'

interface Props { onComplete: (data: Record<string, unknown>) => void }

const CATEGORIES = {
  'Income/Revenue': ['salary', 'revenue', 'income', 'earnings'],
  'Spending/Expenses': ['expenses', 'spending', 'costs', 'bills'],
  'Saving/Investing': ['savings', 'stock', 'investing', 'interest'],
  'Credit/Debt': ['credit', 'debt', 'loan', 'borrow'],
  'Business Terms': ['profit', 'assets', 'liability', 'equity'],
  'Shopping Math': ['tax', 'discount', 'budget', 'total'],
}

const ALL_TERMS = Object.values(CATEGORIES).flat()

export default function Activity61B({ onComplete }: Props) {
  ...
  const handleReveal = () => {
    ...
    ALL_TERMS.forEach(term => {
      correct[term] = getCorrectCategory(term) || ''
    })
    ...
    <div className="p-6 md:p-8">
      <div className="text-center mb-6">
        <div className="flex items-center justify-center gap-2 mb-2">
          ...
          <h2 className="text-xl font-bold text-gray-900">Activity 6.1B – Category Sort</h2>
        </div>
        <p className="text-gray-600">Sort financial terms into the correct categories</p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
        <p className="text-blue-800 text-sm">
          <strong>How to sort:</strong> Click a term, then click a category. Click a placed term to remove it.
        </p>
      </div>

      <div className="bg-gray-100 rounded-xl p-4 mb-6">
        <p className="text-sm font-medium text-gray-700 mb-3">Click a term to select it:</p>
                ...
                {termsInCategory.map(term => {
                  ...
                  return (
                    <button
                      ...
                      } ${revealed ? 'cursor-default' : 'cursor-pointer hover:opacity-80'}`}
                      title={revealed ? '' : 'Click to remove'}
                    >
                      {term}
      ...
      {checked && !allCorrect && !revealed && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
          <p className="text-amber-800 mb-2"><strong>Almost!</strong> Some terms are in the wrong category.</p>
          <p className="text-amber-700 text-sm">💡 Click on any term in a category to remove it and place it correctly!</p>
        </div>
      )}

      {((allCorrect && checked) || revealed) && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6">
          <p className="text-green-800"><strong>🎉 Great job!</strong> You sorted all the terms correctly!</p>
        </div>
      )}
      ...
      <div className="flex justify-center gap-4">
        ...
        {((allCorrect && checked) || revealed) && (
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            onClick={() => onComplete({ activity_61b_attempts: attempts, activity_61b_revealed: revealed })}
