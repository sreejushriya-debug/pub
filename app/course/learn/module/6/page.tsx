...
export default function Module6Interactive() {
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      const clerkProgress = (user.publicMetadata?.courseProgress as Record<string, unknown>)?.module6 as {
  ...
  const saveProgress = (step: Step, data?: Record<string, unknown>) => {
    ...
    if (user?.id) {
      localStorage.setItem(`module6_progress_${user.id}`, JSON.stringify(progressData))
      fetch('/api/progress', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ moduleNumber: 6, progressData }) }).catch(err => console.error('Failed to sync:', err))
    }
  }
