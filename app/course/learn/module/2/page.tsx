...
export default function Module2Interactive() {
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      const clerkProgress = (user.publicMetadata?.courseProgress as Record<string, unknown>)?.module2 as {
  ...
  const saveProgress = (step: Step, data?: Record<string, unknown>) => {
    ...
    if (user?.id) {
      localStorage.setItem(`module2_progress_${user.id}`, JSON.stringify(progressData))
      fetch('/api/progress', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ moduleNumber: 2, progressData })
  ...
  const handleStepComplete = (data?: Record<string, unknown>) => {
    ...
    if (currentIndex < STEPS.length - 1) {
      ...
      const progressData = {
        currentStep: nextStep, completedSteps: newCompletedArray,
        moduleData: { ...moduleData, ...data }, highestReached: newHighest
      }
      if (user?.id) {
        localStorage.setItem(`module2_progress_${user.id}`, JSON.stringify(progressData))
        fetch('/api/progress', {
