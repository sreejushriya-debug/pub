...
const CATEGORIES = {
  'Money Coming In/Out': ['revenue', 'expenses', 'profit', 'loss', 'fixed income'],
  'What a Business Owns/Owes': ['assets', 'liability', 'equity', 'share'],
  'People/Entities': ['investor', 'corporation'],
  'Other Business Concepts': ['business', 'marketing', 'stock'],
}

const ALL_TERMS = Object.values(CATEGORIES).flat()

export default function Activity44B({ onComplete }: Props) {
    ...
    <div className="p-6 md:p-8">
      <div className="text-center mb-6">
        <div className="flex items-center justify-center gap-2 mb-2">
          ...
          <h2 className="text-xl font-bold text-gray-900">Activity 4.4B – Group Business Terms</h2>
        </div>
        <p className="text-gray-600">Sort each business term into the correct category</p>
      </div>
      ...
      {((allCorrect && checked) || revealed) && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6">
          <p className="text-green-800"><strong>🎉 Great job!</strong> You categorized all the business terms correctly!</p>
        </div>
      )}
      ...
      <div className="flex justify-center gap-4">
        ...
        {((allCorrect && checked) || revealed) && (
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            onClick={() => onComplete({ activity_44b_attempts: attempts, activity_44b_revealed: revealed })}
