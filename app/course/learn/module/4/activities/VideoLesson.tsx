...
interface Props { videoId: string; title: string; onComplete: (data?: Record<string, unknown>) => void }

export default function VideoLesson({ videoId, title, onComplete }: Props) {
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600 text-sm mt-1">Watch the video, then continue to the activities</p>
      </div>
      ...
      <div className="flex justify-center">
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => onComplete()} disabled={!hasWatched}
          ...
          Continue to Activities <ArrowRight className="w-5 h-5 ml-2" />
        </motion.button>
      </div>
    </div>
  )
